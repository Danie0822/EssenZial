...
const upload = multerConfig('olores');
...
function validarDatos(nombre, imagen, req, res, next) {
    ...
    return { nombre_olor: nombreValidado, imagen_olor: imagenValidada };
}
...
function validarFormatoActualizar(imagen, nombre, req, res, next) {
    ...
    return {
        nombre_olor: nombreValidado,
        imagen_olor: imagenCategoria
    };
}
...
async function agregar(req, res, next) {
    try {
        const datosValidados = validarDatos(req.body.nombre_olor, req.file ? `uploads/olores/${req.file.filename}` : null, req, res, next);
        await controlador.agregar(datosValidados.nombre_olor, datosValidados.imagen_olor);
...
async function actualizar(req, res, next) {
    try {
        const datosValidados = validarFormatoActualizar(req.file ? `uploads/olores/${req.file.filename}` : null, req.body.nombre_olor, req, res, next);
        const id = validarID(req.body.id_olor, req, res, next);
